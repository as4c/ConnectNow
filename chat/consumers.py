...
KEYS_FOLDER = "Keys"
KEY_FILE = os.path.join(KEYS_FOLDER, "fernet.key")
...
# Create a Fernet instance using the loaded or generated key
...
class ChatConsumer(AsyncJsonWebsocketConsumer):
		...
		try:
			if command == "join":
				...
				if len(content["message"].lstrip()) == 0:
					...
					# message = cipher_suite.encrypt(content["message"].encode())
					await self.send_room(content["room"], content["message"])
	...
	async def chat_message(self, event):
		...
		# message = cipher_suite.decrypt(event["message"].encode()).decode()
		# decrypted_message = cipher_suite.decrypt(event["message"].encode()).decode()
